...
export default class Form {
  ...
  submit(requestType, url, override) {
    ...
    return new Promise((resolve, reject) => {
      let data = this.getData(override)
      if(requestType === 'get') {
        data = {
          params: {
            ...data
      ...
      axios[requestType](url, data, {...config, headers}).then(response => {
        resolve(response)
      }).catch(errors => {
        this.onFail(errors)
  ...
  static isValidationError(e) {
    return e.response && e.response.status === 422
  }
}
